function TeachingMode() {
  ...
  return (
    <section id="teaching-mode" className="bg-slate-900 py-12 text-white">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {steps.map((step) => (
            <article key={step.title} className="rounded-2xl border border-white/20 bg-white/5 p-5">
              ...
              <p className="mt-3 rounded-lg bg-white/10 px-3 py-2 text-sm text-slate-100">阶段产出：{step.output}</p>
            </article>
          ))}
        </div>
      </div>
