  ...
  const grades = [
    {
      ...
      weeks: '12 周',
      outline: ['认识 AI：语音助手、图像识别、智能交通等生活应用', 'Python 基础：变量、分支、循环、简单函数', '趣味项目：猜数字游戏、智能问答小助手'],
    },
    {
      level: '初中阶段（13-15岁）',
      focus: 'AI 理解 + Python 进阶',
      weeks: '16 周',
      outline: ['数据思维：数据采集、清洗与可视化', '机器学习入门：分类、回归、模型训练概念', '综合项目：校园行为数据分析、图片分类器'],
    },
    {
      level: '高中阶段（15-18岁）',
      focus: 'AI 应用 + 项目实战',
      weeks: '18 周',
      outline: ['神经网络基础：感知机、深度学习基本流程', 'AI 工具实战：模型调用、提示词设计、自动化流程', '升学项目：研究型课题与作品集打造'],
    },
  ];

  return (
    <section id="course-system" className="py-12">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          {grades.map((item) => (
            <article key={item.level} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm font-medium text-slate-500">建议学习时长：{item.weeks}</p>
              <p className="mt-1 font-medium text-slate-700">核心方向：{item.focus}</p>
